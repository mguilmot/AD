...
Displays various information about AD users, loops through a list or a file.
...
def UserData(usernames):
    ...
    for i in range(len(usernames)):
        ...
        data+=line
        data+="Homedir: " + str(dictDetails["homedirectory"])
        data+=line
        data+="Loginscript: " + str(dictDetails["loginscript"])
        data+=line
        data+="Name: " + str(dictDetails["displayname"])
        data+=line
        data+="Password last set: " + str(dictDetails["lastset"])
        data+=line
        data+="Last login: " + str(dictDetails["lastlogin"])
        data+=line
        data+="Enabled: " + str(dictDetails["status"])
        data+=line
        if show_groups:
            data+="\nGroup memberships:\n"
